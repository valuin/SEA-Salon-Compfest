...
export default function SignUp({
  ...
  const signUp = async (formData: FormData) => {
    ...
    // Hashing the password - only needed if you have a specific use case for storing this in your own tables
    const saltRounds = 10;
    const hashedPassword = await bcrypt.hash(password, saltRounds);

    // Supabase auth signUp - uses the original password, not the hashedPassword
    ...
    // Insert additional user details into the 'users' table, excluding the password if Supabase handles auth
    const { data, error } = await supabase.from("users").insert([
      { name, email, phone, role: "Customer" }, // Remove password from here if Supabase handles auth
    ]);
  ...
  return (
    <div className="flex-1 flex flex-col w-full px-8 sm:max-w-md justify-center gap-2">
      ...
      <form className="flex-1 flex flex-col w-full justify-center gap-2 text-foreground text-redText">
        ...
        <SubmitButton
          formAction={signUp}
          className="border border-neutral-950/80 rounded-md px-4 py-2 text-foreground mb-2 text-neutral-950 hover:bg-neutral-700 hover:text-primary duration-200"
