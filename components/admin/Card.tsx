...
const Card: React.FC<BranchProps> = ({
  id,
  name,
  location,
  openingTime,
  ...
  const operationalHours = `${openingTime.slice(0, 5)} - ${closingTime.slice(
    0,
    5
  )}`;
  ...
  return (
    ...
    >
      ...
      <h2 className="text-lg font-medium justify-center text-neutral-950 mb-4">
        Services
      </h2>
        ...
        {services && services.length > 0 ? (
          services
            ...
            .map((service) => (
              <div
                key={service.id}
                className="bg-redText/50 shadow-md shadow-redText/60 text-white text-xs py-1 px-2 rounded-full"
              >
                {service.name}
              </div>
            ))
